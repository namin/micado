...
cFNumber F_INF = DBL_MAX;               ///< the largest FNumber

#define Ftype REAL_TYPE                 ///< if flows are ints or floats

#if( Ftype == REAL_TYPE )
 cFNumber F_em = DBL_EPS;               ///< machine precision for FNumber
#endif

/*--------------------------------------------------------------------------*/

typedef double          CNumber;        ///< type of arc flow costs
...
cCNumber C_INF = DBL_MAX;               ///< the largest CNumber

#define Ctype REAL_TYPE                 ///< if costs are ints or floats

#if( Ctype == REAL_TYPE )
 cCNumber C_em = DBL_EPS;               ///< machine precision for CNumber
#endif

/*--------------------------------------------------------------------------*/

typedef double          FONumber;       ///< type of the objective function:
                                        ///< has to hold sums of products of
                                        ///< FNumber(s) by CNumber(s)

typedef const FONumber cFONumber;       ///< a read-only o.f. value

cFONumber FO_INF = DBL_MAX;             ///< the largest FONumber

#if( ( Ftype == REAL_TYPE ) || ( Ctype == REAL_TYPE ) )
 #define FOtype REAL_TYPE
 ///< the objective function will be float if either the flows or the costs
 ///< (or both) are of a float type
#else
 #define FOtype INT_TYPE
 ///< the objective function will be float if either the flows or the costs
 ///< (or both) are of a float type
#endif
...
typedef MFRow          *MFMat;          ///< matrix of Multicommodity flows

typedef const MFNumber cMFNumber;       ///< a read-only Multicommodity flow
typedef cMFNumber     *cMFRow;          ///< read-only array

cMFNumber MF_INF = DBL_MAX;             ///< the largest MFNumber

#define MFtype REAL_TYPE                 ///< if MFNumbers are ints or floats
